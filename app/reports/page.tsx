    ...
    // මෙන්න මචං Excel එක ලස්සන කරන Magic එක
    async function downloadExcel() {
        const workbook = new ExcelJS.Workbook()
        const worksheet = workbook.addWorksheet('Monthly OT Report')

        // 1. Columns ටික හදමු (Header නම් ටික)
        worksheet.columns = [
        ...
        { header: 'Normal Balance', key: 'normalBal', width: 18 },
        { header: 'Friday Balance', key: 'fridayBal', width: 18 },
        ...
        // 3. Header එක ලස්සන කරමු (Style the header row)
        const headerRow = worksheet.getRow(1)
        headerRow.font = { bold: true, color: { argb: 'FFFFFFFF' } } // සුදු පාට අකුරු, Bold
        headerRow.fill = {
        type: 'pattern',
        pattern: 'solid',
        fgColor: { argb: 'FF4F81BD' } // තද නිල් පාට Background එක
        }
        headerRow.alignment = { vertical: 'middle', horizontal: 'center' }

        // 4. Borders දාමු සහ ඉලක්කම් මැදට කරමු (Cell styling)
        ...
        // Total Payable එක තියෙන තීරුව කහ පාටින් Highlight කරමු
        if (rowNumber > 1) {
            row.getCell(9).fill = {
            type: 'pattern',
            pattern: 'solid',
            fgColor: { argb: 'FFFFFF00' } // කහ පාට
            }
            row.getCell(9).font = { bold: true }
        }
        })

        // 5. ෆයිල් එක Save කරමු
    ...
    return (
        ...
        <div className="bg-gray-800 p-6 rounded-xl border border-gray-700 flex gap-4 items-end mb-8 shadow-2xl">
            ...
            <input type="month" value={selectedMonth} onChange={e => setSelectedMonth(e.target.value)} className="w-full bg-gray-700 p-3 rounded border border-gray-600 text-white" />
            </div>
            <button onClick={generateReport} className="bg-blue-600 px-8 py-3 rounded font-bold hover:bg-blue-700 transition">Generate</button>
            {reportData.length > 0 && <button onClick={downloadExcel} className="bg-green-600 px-8 py-3 rounded font-bold hover:bg-green-700 transition">📥 Download Styled Excel</button>}
        </div>
